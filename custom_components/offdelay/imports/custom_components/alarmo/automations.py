...
from .sensors import STATE_CLOSED, STATE_OPEN, STATE_UNAVAILABLE
...
def validate_area(trigger, area_id, hass):
    ...
    if const.ATTR_AREA not in trigger:
        return False
    if trigger[const.ATTR_AREA]:
        return trigger[const.ATTR_AREA] == area_id
    if len(hass.data[const.DOMAIN]["areas"]) == 1:
        return True
...
def validate_modes(trigger, mode):
    ...
    if const.ATTR_MODES not in trigger:
        return False
    if not trigger[const.ATTR_MODES]:
        return True
...
class AutomationHandler:
    ...
    def __init__(self, hass: HomeAssistant):
        ...
        @callback
        async def async_handle_event(event: str, area_id: str, args: dict = None):
            if args is None:
                args = {}
