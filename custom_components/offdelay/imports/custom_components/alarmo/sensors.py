...
def sensor_state_allowed(state, sensor_config, alarm_state):  # noqa: PLR0911
    ...
    if alarm_state == AlarmControlPanelState.TRIGGERED:
    ...
    if alarm_state == AlarmControlPanelState.PENDING:
...
class SensorHandler:
    ...
    def __init__(self, hass: HomeAssistant):
        ...
        def _setup_sensor_listeners():
            ...
            for area_id in self.hass.data[const.DOMAIN]["areas"]:
    ...
    def active_sensors_for_alarm_state(
        ...
        for entity, config in self._config.items():
            if config["area"] != area_id or not config["enabled"] or (
