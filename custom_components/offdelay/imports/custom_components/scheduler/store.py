...
import attr
from homeassistant.const import ATTR_NAME, CONF_CONDITIONS
        ...
        if old_version < 2:
            data["schedules"] = (
                [
                    {
                        **entry,
                        const.ATTR_START_DATE: entry.get(const.ATTR_START_DATE, None),
                        const.ATTR_END_DATE: entry.get(const.ATTR_END_DATE, None),
                    }
