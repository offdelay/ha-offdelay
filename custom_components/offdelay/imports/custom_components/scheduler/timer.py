...
from homeassistant.const import STATE_OFF, STATE_ON, WEEKDAYS
...
class TimerHandler:
    ...
    async def async_start_timer(self):
        ...
        if timestamp is not None:
            ...
            if (timestamp - now).total_seconds() < 0:
                self._timer = None
                _LOGGER.debug(
                    f"Timer of {self.id} is not set because it is in the past ({timestamp})"
                )
            else:
                self._timer = async_track_point_in_time(
                    self.hass, self.async_timer_finished, timestamp
                )
                _LOGGER.debug(f"Timer of {self.id} set for {timestamp}")
    ...
    async def async_timer_finished(self, _time):
        ...
        if not self._timer_is_endpoint:
            ...
            _LOGGER.debug(
                f"Timer {self.id} has reached slot {self.current_slot}"
            )
            ...
            _LOGGER.debug(
                f"Timer {self.id} has reached end of timeslot, resetting.."
            )
            await self.async_start_timer()
    ...
    def day_in_weekdays(self, ts: datetime.datetime) -> bool:
        ...
        if (
            ...
            if const.DAY_TYPE_WEEKEND in self._weekdays:
        ...
        if const.DAY_TYPE_DAILY in self._weekdays or not len(self._weekdays) or const.DAY_TYPE_WORKDAY in self._weekdays and day in workday_list or const.DAY_TYPE_WEEKEND in self._weekdays and day in weekend_list:
            return True
    ...
    def calculate_timestamp(
        ...
        if not res:
            ...
            elif (time_sun + offset).total_seconds() <= 86340:
                ts = ts + offset
            else:
                # prevent offset to shift the time past the extends of the day
        ...
        if iteration > 15:
            _LOGGER.warning(
                f"failed to calculate next timeslot for schedule {self.id}"
            )
            return None
