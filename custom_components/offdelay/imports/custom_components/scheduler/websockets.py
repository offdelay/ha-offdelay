...
class SchedulesListView(HomeAssistantView):
    ...
    url = f"/api/{const.DOMAIN}/list"
    name = f"api:{const.DOMAIN}:list"
...
class SchedulesAddView(HomeAssistantView):
    ...
    url = f"/api/{const.DOMAIN}/add"
    name = f"api:{const.DOMAIN}:add"
...
class SchedulesEditView(HomeAssistantView):
    ...
    url = f"/api/{const.DOMAIN}/edit"
    name = f"api:{const.DOMAIN}:edit"
...
class SchedulesRemoveView(HomeAssistantView):
    ...
    url = f"/api/{const.DOMAIN}/remove"
    name = f"api:{const.DOMAIN}:remove"
...
async def async_register_websockets(hass):
    ...
    websocket_api.async_register_command(
        ...
        websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
            {
                vol.Required("type"): f"{const.DOMAIN}/item",
                vol.Required(const.ATTR_SCHEDULE_ID): cv.string,
            }
        ),
    )
    ...
    websocket_api.async_register_command(
        ...
        websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
            {
                vol.Required("type"): f"{const.DOMAIN}/tags",
            }
        ),
    )
